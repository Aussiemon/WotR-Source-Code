...
AnimationEditor.init = function (self)
...
EditorApi.set_camera = function (self, pos, rot)
	self.freeflight:set_state(pos, rot)
end

AnimationEditor.set_root_driving = function (self, driving)
	self.root_driving = driving
end

AnimationEditor.load_level = function (self, level)
...
AnimationEditor.shutdown = function (self)
...
AnimationEditor.spawn = function (self, unit)
	if self.unit then
		self:unspawn()
	end
...
AnimationEditor.unspawn = function (self)
	World.destroy_unit(self.world, self.unit)

	self.unit = nil
end

AnimationEditor.view_machine = function (self, unit, machine)
	self:spawn(unit)
...
AnimationEditor.view_animation = function (self, unit, animation, loop)
	if loop == nil then
		loop = true
	end
...
AnimationEditor.view_offset_animation = function (self, unit, animation, offset)
...
AnimationEditor.animation_event = function (self, name)
...
AnimationEditor.set_variable = function (self, name, value)
	if self.animation then
		return
	end
...
AnimationEditor.set_constraint_target = function (self, name, value)
...
AnimationEditor.set_animation_time = function (self, t)
	if t < 0 then
		t = 0
	end
...
AnimationEditor.play = function (self, speed)
...
AnimationEditor.render = function (self)
...
AnimationEditor.draw_selected_node = function (self, unit, node, lines)
	local w = Unit.world_pose(unit, node)

	LineObject.add_axes(lines, w, 0.3)
end

AnimationEditor.update = function (self, dt)
...
AnimationEditor.draw_flash_text = function (self, dt)
...
AnimationEditor.draw_skeleton = function (self)
...
AnimationEditor.pick_bone = function (self)
	if not self.unit then
		return
	end
...
AnimationEditor.draw_box = function (self, tm, radius, color)
...
AnimationEditor.mouse_ray = function (self)
...
AnimationEditor.pick_constraint_target = function (self)
	local cam, dir = self:mouse_ray()
...
AnimationEditor.draw_constraint_targets = function (self)
	local cam, dir = self:mouse_ray()
...
AnimationEditor.mouse_down = function (self)
	if self.options.draw_constraint_targets then
...
AnimationEditor.mouse_up = function (self)
	self.drag = nil
end

AnimationEditor.drag_pos = function (self, start_pos, cam_delta, lock)
...
AnimationEditor.do_drag = function (self)
...
AnimationEditor.activated = function (self, dt)
	if Window then
		Window.set_focus()
	end
end

AnimationEditor.focus = function (self)
	if Window then
		Window.set_focus()
		Window.set_mouse_focus(true)
	end
...
AnimationEditor.reset_to_origin = function (self)
	if self.unit then
		Unit.set_local_position(self.unit, 0, Vector3(0, 0, 0))
	end
end

AnimationEditor.flash = function (self, s, full_time, fade_time)
	self.flash_text = s
	self.flash_age = 0
