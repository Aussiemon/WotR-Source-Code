...
URLTextureMenuItem.init = function (self, config, world)
	URLTextureMenuItem.super.init(self, config, world)
end

URLTextureMenuItem.update_size = function (self, dt, t, gui, layout_settings)
...
URLTextureMenuItem.update_position = function (self, dt, t, layout_settings, x, y, z)
	self._x = x
	self._y = y
	self._z = z or 1
end

URLTextureMenuItem.render = function (self, dt, t, gui, layout_settings)
...
URLTextureMenuItem.create_from_config = function (compiler_data, config, callback_object)
	local config = {
		type = "url_texture",
		page = config.page,
		name = config.name,
