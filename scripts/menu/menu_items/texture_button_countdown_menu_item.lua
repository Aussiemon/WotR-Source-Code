...
TextureButtonCountdownMenuItem.init = function (self, config, world)
	TextureButtonCountdownMenuItem.super.init(self, config, world)
end

TextureButtonCountdownMenuItem.on_page_enter = function (self, on_cancel)
	TextureButtonCountdownMenuItem.super.on_page_enter(self, on_cancel)

	self._timer = self.config.countdown_time
end

TextureButtonCountdownMenuItem.set_text = function (self, text)
	self.config.original_text = text
end

TextureButtonCountdownMenuItem.update_size = function (self, dt, t, gui, layout_settings)
...
TextureButtonCountdownMenuItem.create_from_config = function (compiler_data, config, callback_object)
	local config = {
		type = "texture_button",
		name = config.name,
		page = config.page,
