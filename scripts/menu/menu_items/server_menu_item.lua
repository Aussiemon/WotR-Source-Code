...
ServerMenuItem.init = function (self, config, world)
...
ServerMenuItem.server = function (self)
	return self._server
end

ServerMenuItem.on_select = function (self, ignore_sound)
	self:set_selected(true)
	ServerMenuItem.super.on_select(self, ignore_sound)
end

ServerMenuItem.on_deselect = function (self)
	self:set_selected(false)
end

ServerMenuItem.set_selected = function (self, selected)
	self._selected = selected
end

ServerMenuItem.selected = function (self)
	return self._selected
end

ServerMenuItem.set_favorite = function (self, favorite)
	self._is_favorite = favorite
end

ServerMenuItem.is_favorite = function (self)
	return self._is_favorite
end

ServerMenuItem.sort_value = function (self, key)
	return self._sort_values[key]
end

ServerMenuItem.update_size = function (self, dt, t, gui, layout_settings, w, h)
	self._width = w
	self._height = h
end

ServerMenuItem.update_position = function (self, dt, t, layout_settings, x, y, z)
	self._x = x
	self._y = y
	self._z = z
end

ServerMenuItem._update_sort_values = function (self, server)
...
ServerMenuItem.set_hidden = function (self, hidden)
	ServerMenuItem.super.set_hidden(self, hidden)

	self.config.favorite_item.config.hidden = hidden
end

ServerMenuItem.render = function (self, dt, t, gui, layout_settings)
...
ServerMenuItem._update_favorite_item = function (self, dt, t, gui, column_offset_x, layout_settings, is_favorite)
...
ServerMenuItem._num_friends = function (self)
	local num_friends = 0
...
ServerMenuItem.visible_in_demo = function (self)
	return IS_DEMO and not DemoSettings.available_game_modes[self._server.game_mode_key]
end

ServerMenuItem.create_from_config = function (compiler_data, config, callback_object)
	local config = {
		type = "server",
		page = config.page,
		name = config.name,
