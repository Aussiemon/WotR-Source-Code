...
TextureButtonMenuItem.init = function (self, config, world)
	TextureButtonMenuItem.super.init(self, config, world)
end

TextureButtonMenuItem.set_text = function (self, text)
	self.config.text = text
end

TextureButtonMenuItem.start_pulse = function (self, speed)
	self._pulse = {
		speed = speed
	}
end

TextureButtonMenuItem.stop_pulse = function (self)
	self._pulse = nil
end

TextureButtonMenuItem.pulse = function (self)
	return self._pulse
end

TextureButtonMenuItem.update_size = function (self, dt, t, gui, layout_settings)
...
TextureButtonMenuItem.update_position = function (self, dt, t, layout_settings, x, y, z)
	self._x = x + (layout_settings.texture_offset_x or 0)
	self._y = y + (layout_settings.texture_offset_y or 0)
	self._z = z
end

TextureButtonMenuItem.render_from_child_page = function (self, dt, t, gui, layout_settings)
	self:render(dt, t, gui, layout_settings)
end

TextureButtonMenuItem.render = function (self, dt, t, gui, layout_settings)
...
TextureButtonMenuItem.create_from_config = function (compiler_data, config, callback_object)
	local config = {
		type = "texture_button",
		name = config.name,
		page = config.page,
