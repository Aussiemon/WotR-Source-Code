...
EnumMenuItem.init = function (self, config, world)
...
EnumMenuItem.update_size = function (self, dt, t, gui, layout_settings)
...
EnumMenuItem.update_position = function (self, dt, t, layout_settings, x, y, z)
	self._x = x
	self._y = y
	self._z = z
end

EnumMenuItem.render_from_child_page = function (self, dt, t, gui, layout_settings)
	local config = self.config
...
EnumMenuItem.render = function (self, dt, t, gui, layout_settings)
	local config = self.config
...
EnumMenuItem.select_entry_by_key = function (self, entry_key)
	local selection
...
EnumMenuItem.on_left_click = function (self, ignore_sound)
	self:on_move_right()
...
EnumMenuItem.on_right_click = function (self, ignore_sound)
	self:on_move_left()
...
EnumMenuItem.on_move_left = function (self)
...
EnumMenuItem.on_move_right = function (self)
	self._selection = self._selection + 1
...
EnumMenuItem.on_page_enter = function (self)
...
EnumMenuItem._update_and_notify_value = function (self, new_entry)
...
EnumMenuItem.notify_value = function (self, ...)
	self:_try_callback(self.config.callback_object, self.config.on_option_changed, self.config.selected_entry, ...)
end

EnumMenuItem.create_from_config = function (compiler_data, config, callback_object)
	local config = {
		type = "enum",
		page = config.page,
		name = config.name,
