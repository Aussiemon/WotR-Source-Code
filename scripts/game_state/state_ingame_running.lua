...
ObserverCameraController.init = function (self, player)
	self._player = player
...
ObserverCameraController._pick_observer_target = function (self, dt, t, last_id, own_player, input)
...
ObserverCameraController._first_target = function (self, targets, own_player)
...
ObserverCameraController._next_target = function (self, targets, current, own_player)
...
ObserverCameraController._last_target = function (self, targets, current, own_player)
	local found_current, target
...
ObserverCameraController._get_targets = function (self)
...
ObserverCameraController.update = function (self, dt, t, player, viewport_name, camera_manager, input)
...
ObserverCameraController.destroy = function (self)
	return
end

StateInGameRunning.gm_event_end_conditions_met = function (self, winning_team_name, red_team_score, white_team_score, end_of_round_only)
...
StateInGameRunning.event_gained_xp_and_coins = function (self, reason, xp, coins)
...
StateInGameRunning.event_penalty_xp = function (self, reason, amount)
...
StateInGameRunning.event_awarded_prize = function (self, name)
...
StateInGameRunning.event_awarded_medal = function (self, name)
...
StateInGameRunning.event_awarded_rank = function (self, rank)
	self._awarded_ranks[#self._awarded_ranks + 1] = rank
end

StateInGameRunning._set_battle_report_data = function (self, local_player_won)
	local players = {}
...
StateInGameRunning._setup_camera = function (self)
	local viewport_name = self.viewport_name
...
StateInGameRunning._setup_menus = function (self)
...
StateInGameRunning.event_set_default_camera_pose = function (self, position, rotation)
	self._default_camera_position:store(position)
	self._default_camera_rotation:store(rotation)
end

StateInGameRunning._set_default_camera = function (self, camera_manager, viewport_name)
...
StateInGameRunning._update_environment = function (self, dt, t)
	local player = self.player
...
StateInGameRunning._update_input = function (self, dt, t, input)
	local player = self.player
...
StateInGameRunning._update_locomotion_input = function (self, dt)
...
StateInGameRunning._activate_chat_input = function (self, input, prefix, try_big_picture)
...
StateInGameRunning._execute_chat_input = function (self)
	self._chat_input_active = false
...
StateInGameRunning._execute_controller_chat_input = function (self, text)
...
StateInGameRunning._deactivate_chat_input = function (self, shutdown_light)
...
StateInGameRunning._modify_chat_input_blackboard = function (self, dt, t, input)
...
KeystrokeHelper._build_utf8_table = function (text)
	local text_table = {}
...
StateInGameRunning.event_player_killed_by_enemy = function (self, own_player, attacking_player)
...
StateInGameRunning._update_camera = function (self, dt, t, input)
	local player = self.player
...
StateInGameRunning._update_camera_target = function (self, dt, t, player, viewport_name, camera_manager, input, wants_observer)
	local current_cam_unit = camera_manager:current_node_tree_root_unit(viewport_name)
...
StateInGameRunning.event_dead_player_unit_despawned = function (self, player)
...
StateInGameRunning.event_game_started = function (self, skip_team_selection)
...
StateInGameRunning.event_join_team_confirmed = function (self)
...
StateInGameRunning.event_ghost_mode_deactivated = function (self)
	self._ingame_menu:set_active(false)
end
...
StateInGameRunning.event_force_close_ingame_menu = function (self)
	self._force_close_ingame_menu = true
end

StateInGameRunning.event_spawn_target_denied = function (self)
	if not self._final_scoreboard_menu:active() then
		self._scoreboard:set_active(false)
		self._ingame_menu:set_active(true)
		self._ingame_menu:goto("select_spawnpoint")
...
StateInGameRunning.event_level_ended = function (self)
...
StateInGameRunning._save_level_progression_id = function (self, id)
...
StateInGameRunning.cb_player_data_saved = function (self, info)
	if info.error then
...
StateInGameRunning.ingame_menu = function (self)
	return self._ingame_menu
end

StateInGameRunning._update_director_mode = function (self, dt, t)
...
StateInGameRunning.ingame_menu_cancel_to = function (self, page_id)
