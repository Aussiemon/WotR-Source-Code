...
HUDHelper.layout_settings = function (self, settings_table)
...
HUDHelper.element_position = function (self, parent_element, element, layout_settings)
...
HUDHelper.floating_icon_size = function (self, screen_z, texture_width, texture_height, min_scale, max_scale, min_scale_distance, max_scale_distance)
...
HUDHelper.floating_text_icon_size = function (self, screen_z, font_size, min_scale, max_scale, min_scale_distance, max_scale_distance)
	local k = (max_scale - min_scale) / (max_scale_distance - min_scale_distance)
	local scale = k * (screen_z - max_scale_distance) + max_scale
...
HUDHelper.clamped_icon_position = function (self, x, y, z)
	local res_width, res_height = Gui.resolution()
...
HUDHelper.inside_attention_screen_zone = function (self, x, y, z)
	if z > 0 then
		local res_width, res_height = Gui.resolution()
...
HUDHelper.inside_default_screen_zone = function (self, x, y, z)
	if z > 0 then
		local res_width, res_height = Gui.resolution()
		local margin_x = res_height * self.STANDARD_ASPECT_RATIO * (1 - HUDSettings.default_zone.x_radius)
		local margin_y = res_height * (1 - HUDSettings.default_zone.y_radius)
...
HUDHelper.inside_screen = function (self, x, y, z)
	if z > 0 then
		local res_width, res_height = Gui.resolution()
...
HUDHelper.crop_text = function (self, gui, text, font, font_size, max_width, crop_suffix)
	local suffix = ""
...
HUDHelper.trunkate_text = function (self, text, max_length, trunkate_suffix, replace_all_with_suffix)
...
HUDHelper.player_color = function (self, local_player, other_player)
	local color_table
...
HUDHelper.render_version_info = function (self, gui)
	local res_width, res_height = Gui.resolution()
	local text_size = 18
...
HUDHelper.render_fps = function (self, gui, dt)
	local fps
...
HUDHelper.floating_hud_icon_color = function (player, settings, blackboard, dt, t)
	local color_table = {
		255,
		255,
		255,
...
HUDHelper.atlas_material = function (atlas_name, material_name, masked)
