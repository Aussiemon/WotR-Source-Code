...
HorseLocomotion.init = function (self, world, unit, id, game, ghost_mode, spawner_unit_id)
	self.world = world
	self.game = game
	self.id = id
	self.unit = unit
...
HorseLocomotion.fall_height = function (self)
	if self.current_state.fall_height then
		return self.current_state:fall_height()
	end
end

HorseLocomotion._setup_mount_profile = function (self)
...
HorseLocomotion._create_game_object = function (self)
	local unit = self.unit
...
HorseLocomotion.cb_game_session_disconnect = function (self)
	self._frozen = true
	self.id = nil
	self.game = nil
end

HorseLocomotion._init_state_values = function (self)
	self.rot_speed = 0
...
HorseLocomotion.switch_to_local = function (self)
	self._husk = false
...
HorseLocomotion.switch_to_husk = function (self)
	self._husk = true
...
HorseLocomotion._set_spawn_position = function (self, unit)
	local mover = Unit.mover(unit)
...
HorseLocomotion.update = function (self, unit, input, dt, context, t)
...
HorseLocomotion._update_collision_sweep = function (self, dt, t)
...
HorseLocomotion.impact_obstacle = function (self)
	if self.current_state.impact_obstacle then
		self.current_state:impact_obstacle()
	end
end

HorseLocomotion._update_despawn_timer = function (self, dt, t)
	local unit = self.unit
...
HorseLocomotion._spawner_in_range = function (self, spawner_unit)
...
HorseLocomotion._enter_ghost_mode = function (self)
...
HorseLocomotion.exit_ghost_mode = function (self)
...
HorseLocomotion.destroy = function (self)
	local unit = self.unit
...
HorseLocomotion.create_state = function (self, state_name, new_state)
	self._states[state_name] = new_state:new(self.unit, self)
end

HorseLocomotion.change_state = function (self, new_state)
	self.current_state:change_state(new_state)
end

HorseLocomotion.set_init_state = function (self, new_state)
...
HorseLocomotion.anim_cb = function (self, callback, unit, param)
...
HorseLocomotion.anim_cb_landing_to_onground = function (self)
	return
end

HorseLocomotion.rpc_set_dead = function (self, impact_direction)
	self:set_dead(impact_direction)
end

HorseLocomotion.set_dead = function (self, impact_direction)
...
HorseLocomotion.get_velocity = function (self)
	return self.velocity:unbox()
end

HorseLocomotion.rpc_animation_event = function (self, event)
	local unit = self.unit
...
HorseLocomotion.steering = function (self)
	return self._steering
end

HorseLocomotion.stun = function (self)
	return
end

HorseLocomotion.damage_interrupt = function (self)
	return
end

HorseLocomotion.received_damage = function (self)
	return
end

HorseLocomotion.hurt_sound_event = function (self)
	return nil
end

HorseLocomotion.has_perk = function (self)
	return false
end

HorseLocomotion.rpc_animation_set_variable = function (self, index, variable)
	Unit.animation_set_variable(self.unit, index, variable)
end

HorseLocomotion.hot_join_synch = function (self, sender, player)
	local unit = self.unit

	RPC.rpc_synch_horse_anim_state(sender, self.id, Unit.animation_get_state(unit))
end

HorseLocomotion.teleport = function (self, position, rotation)
