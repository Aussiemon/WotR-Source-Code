...
WeaponShield.init = function (self, world, unit, user_unit, player, id)
	self._world = world
	self._unit = unit
...
WeaponShield.update = function (self, dt, t)
...
WeaponShield.start_reload = function (self)
	return
end

WeaponShield.can_wield = function (self)
	return true
end

WeaponShield.category = function (self)
	return self._weapon_category
end

WeaponShield.wield_finished_anim_name = function (self)
	return
end

WeaponShield.set_wielded = function (self, wielded)
	return
end

WeaponShield.start_attack = function (self, charge_time, attack_name, quick_swing, abort_attack_func)
...
WeaponShield.end_attack = function (self)
	self._attacking = false
...
WeaponShield._activate_sweep_collision = function (self)
...
WeaponShield._update_sweep_collision = function (self, dt, sweep_settings)
...
WeaponShield._calculate_sweep_extents = function (self, sweep_settings)
	local unit = self._unit
...
WeaponShield._deactivate_sweep_collision = function (self)
	self._sweep_collision = false
end

WeaponShield._abort_attack = function (self, reason)
...
WeaponShield.debug_draw_sweep_area = function (self)
	if script_data.weapon_collision_debug then
		local drawer = Managers.state.debug:drawer({
			mode = "immediate",
...
WeaponShield.shield_bash_hit_cb = function (self, hit_unit, actor, normal, position)
...
WeaponShield._shield_bash_hit_character = function (self, hit_unit, position, normal, actor, hit_zone, impact_direction)
	local current_attack = self._current_attack
...
WeaponShield.hot_join_synch = function (self, sender, player, player_object_id, slot_name)
	return
end

WeaponShield.enter_ghost_mode = function (self)
	self._ghost_mode = true
end

WeaponShield.exit_ghost_mode = function (self)
	self._ghost_mode = false
end

WeaponShield.destroy = function (self)
	return
end
