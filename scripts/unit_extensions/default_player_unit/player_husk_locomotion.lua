...
PlayerHuskLocomotion.init = function (self, world, unit, id, game, ghost_mode)
	self._world = world
	self._unit = unit
	self._game = game
	self._id = id
...
PlayerHuskLocomotion._enter_ghost_mode = function (self)
...
PlayerHuskLocomotion._exit_ghost_mode = function (self)
...
PlayerHuskLocomotion.set_ladder_unit = function (self, ladder_unit)
	self.ladder_unit = ladder_unit
end

PlayerHuskLocomotion.rpc_add_instakill_push = function (self, velocity, mass, hit_zone_name)
...
PlayerHuskLocomotion._apply_instakill_push = function (self, instakill_table)
...
PlayerHuskLocomotion.setup_player_profile = function (self, obj_id)
...
PlayerHuskLocomotion.has_perk = function (self, perk_name)
...
PlayerHuskLocomotion.update = function (self, unit, input, dt, context)
...
PlayerHuskLocomotion._update_collision = function (self, dt, unit, movement_state)
...
PlayerHuskLocomotion._update_culling = function (self, dt)
	local unit = self._unit
...
PlayerHuskLocomotion._update_bone_lod = function (self, dt, unit, distance)
...
PlayerHuskLocomotion._update_physics_culling = function (self, dt, unit, distance)
...
PlayerHuskLocomotion.rpc_gear_dead = function (self, unit)
	self:gear_dead(unit)
end

PlayerHuskLocomotion.gear_dead = function (self, unit)
	local inventory = self._inventory
...
PlayerHuskLocomotion.update_movement = function (self, dt, unit, movement_state)
...
PlayerHuskLocomotion._dismount_movement = function (self, unit, dt, old_pos, new_pos, new_rot, movement_state, mounted, velocity)
...
PlayerHuskLocomotion._mounted_movement = function (self, unit, dt, old_pos, new_pos, new_rot, movement_state, mounted, velocity)
	self._pos_lerp_time = 0
...
PlayerHuskLocomotion._extrapolation_movement = function (self, unit, dt, old_pos, new_pos, new_rot, movement_state, mounted, velocity)
...
PlayerHuskLocomotion._update_speed_variable = function (self)
...
PlayerHuskLocomotion._calculate_move_speed_var_from_mps = function (self, move_speed)
	local speed_var
...
PlayerHuskLocomotion.update_aim_target = function (self, dt, unit)
...
PlayerHuskLocomotion.set_mounted = function (self, mounted)
...
PlayerHuskLocomotion.mounted = function (self)
	return self._mounted ~= nil
end

PlayerHuskLocomotion.destroy = function (self, u, input)
	local unit = self._unit
	local player_manager = Managers.player
...
PlayerHuskLocomotion._setup_inventory = function (self, world, unit, player, tint_color, secondary_tint_color, coat_of_arms)
	self._inventory = PlayerHuskInventory:new(world, unit, player, tint_color, secondary_tint_color, coat_of_arms)
end

PlayerHuskLocomotion.inventory = function (self)
	return self._inventory
end

PlayerHuskLocomotion.anim_cb = function (self, callback, unit, param)
...
PlayerHuskLocomotion.anim_cb_hide_wielded_weapons = function (self)
	self._inventory:anim_cb_hide_wielded_weapons(true)
end

PlayerHuskLocomotion.anim_cb_unhide_wielded_weapons = function (self)
	self._inventory:anim_cb_hide_wielded_weapons(false)
end

PlayerHuskLocomotion.player_knocked_down = function (self)
	self._inventory:set_kinematic_wielded_gear(false)
end

PlayerHuskLocomotion.player_dead = function (self, is_instakill, damage_type, hit_zone, impact_direction)
	local inventory = self._inventory
...
PlayerHuskLocomotion.rpc_start_revive = function (self)
	return
end

PlayerHuskLocomotion.rpc_abort_revive = function (self)
	return
end

PlayerHuskLocomotion.rpc_completed_revive = function (self)
	self._inventory:set_kinematic_wielded_gear(true)
end

PlayerHuskLocomotion.rpc_raise_parry_block = function (self, slot_name, direction, delay)
...
PlayerHuskLocomotion.rpc_lower_parry_block = function (self)
...
PlayerHuskLocomotion.rpc_hot_join_synch_parry_block = function (self, slot_name, direction)
	self.block_slot_name = slot_name
	self.block_unit = self._inventory:gear_unit(slot_name)
	self.block_direction = direction
...
PlayerHuskLocomotion.rpc_raise_shield_block = function (self, slot_name)
	self.block_slot_name = slot_name
	self.block_unit = self._inventory:gear_unit(slot_name)
	self.blocking = true
...
PlayerHuskLocomotion.rpc_lower_shield_block = function (self)
	self.block_slot_name = nil
	self.block_unit = nil
	self.blocking = false
...
PlayerHuskLocomotion.rpc_hot_join_synch_shield_block = function (self, slot_name)
	self.block_slot_name = slot_name
	self.block_unit = self._inventory:gear_unit(slot_name)
	self.block_raised_time = 0
	self.blocking = true
...
PlayerHuskLocomotion.rpc_pose_melee_weapon = function (self, direction)
	self.pose_direction = direction
	self.posing = true
end

PlayerHuskLocomotion.rpc_stop_posing_melee_weapon = function (self)
	self.pose_direction = nil
	self.posing = false
end

PlayerHuskLocomotion.rpc_hot_join_synch_pose = function (self, direction)
	self.pose_direction = direction
	self.posing = true
end

PlayerHuskLocomotion.get_velocity = function (self)
...
PlayerHuskLocomotion.rpc_flag_pickup_confirmed = function (self, flag_unit)
	self.carried_flag = flag_unit
end

PlayerHuskLocomotion.rpc_flag_plant_complete = function (self, flag_unit)
	assert(self.carried_flag == flag_unit, "Trying to plant flag not carried.")

	self.carried_flag = nil
end

PlayerHuskLocomotion.rpc_drop_flag = function (self, flag_unit)
	self.carried_flag = nil
...
PlayerHuskLocomotion.rpc_animation_event = function (self, event)
	Unit.animation_event(self._unit, event)
end

PlayerHuskLocomotion.rpc_animation_set_variable = function (self, index, variable)
	Unit.animation_set_variable(self._unit, index, variable)
end

PlayerHuskLocomotion.hot_join_synch = function (self, sender, player)
...
PlayerHuskLocomotion.stun = function (self)
...
PlayerHuskLocomotion.received_damage = function (self)
	return
end

PlayerHuskLocomotion.damage_interrupt = function (self, hit_zone, impact_direction, impact_type)
	WeaponHelper:player_unit_hit_reaction_animation(self._unit, hit_zone, impact_direction, self:aim_direction(), impact_type)
end

PlayerHuskLocomotion.aim_direction = function (self)
	return GameSession.game_object_field(self._game, self._id, "aim_target")
end

PlayerHuskLocomotion._attachment_node_linking = function (self, source_unit, target_unit, linking_setup)
...
PlayerHuskLocomotion.abort_execution_victim = function (self)
	ExecutionHelper.play_execution_abort_anim(self._unit)
end

PlayerHuskLocomotion.start_execution_victim = function (self, execution_definition, attacker_unit)
	ExecutionHelper.play_execution_victim_anims(self._unit, attacker_unit, execution_definition)
end

PlayerHuskLocomotion.start_execution_attacker = function (self, execution_definition, victim_unit)
	ExecutionHelper.play_execution_attacker_anims(self._unit, victim_unit, execution_definition)
end

PlayerHuskLocomotion.hurt_sound_event = function (self)
	return "husk_vce_hurt"
end

PlayerHuskLocomotion.post_update = function (self, dt)
