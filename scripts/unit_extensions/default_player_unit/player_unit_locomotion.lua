...
PlayerUnitLocomotion._force_unit_to_ground = function (self, unit)
...
PlayerUnitLocomotion.init = function (self, world, unit, player_index, ghost_mode, profile)
...
PlayerUnitLocomotion._setup_debug_variables = function (self, unit)
...
PlayerUnitLocomotion._setup_states = function (self)
...
PlayerUnitLocomotion._create_game_object = function (self, unit, ghost_mode)
	local mover = Unit.mover(unit)
...
PlayerUnitLocomotion._setup_player_profile = function (self, profile)
...
PlayerUnitLocomotion.spawn_new_mount = function (self, player, mount_profile, unit, ghost_mode, cavalry_perk_used)
...
PlayerUnitLocomotion.has_perk = function (self, perk_name, perks)
	if perks then
...
PlayerUnitLocomotion._setup_team_dependants = function (self, unit, player)
...
PlayerUnitLocomotion._setup_inventory = function (self, player, spawn_horse)
	local tint_color = {
		1,
		0,
		0.5
...
PlayerUnitLocomotion._enter_ghost_mode = function (self)
...
PlayerUnitLocomotion.blend_out_of_ghost_mode = function (self)
	self._ghost_mode_blend_timer = Managers.time:time("game")
end

PlayerUnitLocomotion.exit_ghost_mode = function (self)
...
PlayerUnitLocomotion._disable_team_specific_actors = function (self)
...
PlayerUnitLocomotion.cb_game_session_disconnect = function (self)
	self:_freeze()
...
PlayerUnitLocomotion.inventory = function (self)
	return self._inventory
end

PlayerUnitLocomotion._init_internal_variables = function (self, unit, player)
	local rot = Unit.local_rotation(unit, 0)
	local pos = Unit.local_position(unit, 0)
	local t = Managers.time:time("game")
...
PlayerUnitLocomotion._husks_in_proximity_overlap = function (self, dt, t)
...
PlayerUnitLocomotion.husks_in_proximity_overlap_cb = function (self, actors)
...
PlayerUnitLocomotion.update = function (self, unit, input, dt, context, t)
	if self.frozen then
		return
	end
...
PlayerUnitLocomotion._update_in_combat = function (self, dt, t)
...
PlayerUnitLocomotion._update_leave_ghost_mode = function (self, dt, t)
...
PlayerUnitLocomotion.post_update = function (self, unit, input, dt, context, t)
	if self.current_state.post_world_update then
		self.current_state:post_world_update(dt, t)
	end
end

PlayerUnitLocomotion._update_state_data = function (self, dt, t)
	local player = self.player
...
PlayerUnitLocomotion.destroy = function (self)
...
PlayerUnitLocomotion.force_mount_unit = function (self, mount_unit)
	self.mounted_unit = mount_unit

	self:_change_state("mounted")
end

PlayerUnitLocomotion.rpc_mount_denied = function (self, mount_object_id, mount_unit)
	if self.current_state_name == "mounted" and self.mounted_unit == mount_unit then
		self.current_state:force_unmount("mount_denied")
	end
end

PlayerUnitLocomotion.rpc_unmount = function (self, mount_unit)
	self:unmount(mount_unit)
end

PlayerUnitLocomotion.unmount = function (self, mount_unit)
	if self.current_state_name == "mounted" and self.mounted_unit == mount_unit then
		self.current_state:force_unmount("unmount")
	end
end

PlayerUnitLocomotion.mounted = function (self)
	return self.current_state_name == "mounted"
end

PlayerUnitLocomotion.stun = function (self, hit_zone, impact_direction, impact_type, ...)
...
PlayerUnitLocomotion.damage_interrupt = function (self, hit_zone, impact_direction, impact_type)
...
PlayerUnitLocomotion.received_damage = function (self, damage, front_back, direction)
	local t = Managers.time:time("game")
...
PlayerUnitLocomotion.rpc_start_revive = function (self)
	assert(self.current_state_name == "knocked_down")
	self.current_state:start_revive()
end

PlayerUnitLocomotion.rpc_abort_revive = function (self)
	assert(self.current_state_name == "knocked_down")
	self.current_state:abort_revive()
end

PlayerUnitLocomotion.rpc_completed_revive = function (self)
	assert(self.current_state_name == "knocked_down")
	self.current_state:revived()
end

PlayerUnitLocomotion.rpc_gear_dead = function (self, unit)
	self:gear_dead(unit)
end

PlayerUnitLocomotion.player_knocked_down = function (self, hit_zone, impact_direction, damage_type)
	self:_change_state("knocked_down", hit_zone, impact_direction, damage_type)
end

PlayerUnitLocomotion.player_dead = function (self, is_instakill, damage_type, hit_zone, impact_direction)
	self:_change_state("dead", is_instakill, damage_type, hit_zone, impact_direction)
end

PlayerUnitLocomotion.get_velocity = function (self)
	return self.velocity:unbox()
end

PlayerUnitLocomotion.rpc_flag_plant_complete = function (self, flag_unit)
	return
end

PlayerUnitLocomotion.rpc_flag_pickup_confirmed = function (self, flag_unit)
	self.picking_flag = false
	self.carried_flag = flag_unit
end

PlayerUnitLocomotion.rpc_flag_drop = function (self, flag_unit)
	assert(flag_unit == self.carried_flag, "Trying to drop other flag than the one carried.")

	self.carried_flag = nil
end

PlayerUnitLocomotion.rpc_flag_pickup_denied = function (self)
	self.picking_flag = false
end

PlayerUnitLocomotion.rpc_flag_plant_confirmed = function (self)
...
PlayerUnitLocomotion.rpc_flag_plant_denied = function (self)
	if self.current_state_name == "planting_flag" then
		self:_change_state("onground")
	end
end

PlayerUnitLocomotion.hot_join_synch = function (self, sender, player)
	local network_manager = Managers.state.network
...
PlayerUnitLocomotion.yield_interaction_confirmed = function (self)
...
PlayerUnitLocomotion.yield_interaction_denied = function (self)
	if self.current_state_name == "knocked_down" then
		self.current_state:yield_denied()
	end
end

PlayerUnitLocomotion.execute_interaction_confirmed = function (self)
	if self.current_state_name == "executing" then
		self.current_state:interaction_confirmed()
	end
end

PlayerUnitLocomotion.execute_interaction_denied = function (self)
	if self.current_state_name == "executing" then
...
PlayerUnitLocomotion.bandage_interaction_confirmed = function (self)
	if self.current_state_name == "bandaging_teammate" or self.current_state_name == "bandaging_self" then
		self.current_state:interaction_confirmed()
	end
end

PlayerUnitLocomotion.bandage_interaction_denied = function (self)
	if self.current_state_name == "bandaging_teammate" or self.current_state_name == "bandaging_self" then
		self.current_state:interaction_denied()
		self:_change_state("onground")
	end
...
PlayerUnitLocomotion.revive_interaction_confirmed = function (self)
	if self.current_state_name == "reviving_teammate" then
		self.current_state:interaction_confirmed()
	end
end

PlayerUnitLocomotion.revive_interaction_denied = function (self)
	if self.current_state_name == "reviving_teammate" then
		self.current_state:interaction_denied()
		self:_change_state("onground")
	end
...
PlayerUnitLocomotion.trigger_interaction_confirmed = function (self)
	if self.current_state_name == "triggering" then
		self.current_state:interaction_confirmed()
	end
end

PlayerUnitLocomotion.trigger_interaction_denied = function (self)
	if self.current_state_name == "triggering" then
		self.current_state:interaction_denied()
		self:_change_state("onground")
	end
...
PlayerUnitLocomotion.aim_direction = function (self)
	return self.current_state:aim_direction()
end

PlayerUnitLocomotion.start_execution_victim = function (self, execution_definition, attacker_unit)
	self.current_state:begin_execution(execution_definition, attacker_unit)
end

PlayerUnitLocomotion.abort_execution_victim = function (self)
...
PlayerUnitLocomotion.event_teleport_all_to = function (self, position, rotation, camera_rotation)
	self:_teleport_to(position, rotation, camera_rotation)
end

PlayerUnitLocomotion.event_teleport_unit_to = function (self, unit, position, rotation, camera_rotation)
	if self.unit == unit then
		self:_teleport_to(position, rotation, camera_rotation)
	end
end

PlayerUnitLocomotion.event_teleport_team_to = function (self, team_name, position, rotation, camera_rotation)
	if team_name == self.player.team.name then
		self:_teleport_to(position, rotation, camera_rotation)
	end
end

PlayerUnitLocomotion._teleport_to = function (self, position, rotation, camera_rotation)
	if self.current_state_name == "dead" then
		return
	end
...
PlayerUnitLocomotion.set_ladder_unit = function (self, ladder_unit)
	return
end
