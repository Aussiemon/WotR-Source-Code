...
PlayerUnitDamage.init = function (self, world, unit, player_index)
	self._world = world
	self._unit = unit
	self._damage = 0
	self._wounded = false
...
PlayerUnitDamage._setup_hit_zones = function (self, hit_zones)
	local actor_table = {}
...
PlayerUnitDamage._create_game_object = function (self)
	local data_table = {
		burning_end_time = 0,
		bleeding_end_time = 0,
		arrow_level = 0,
...
PlayerUnitDamage.cb_game_session_disconnect = function (self)
	self._frozen = true

	self:remove_game_object_id()
end

PlayerUnitDamage.set_invulnerable = function (self, invulnerable)
	self._invulnerable = invulnerable
end

PlayerUnitDamage.set_game_object_id = function (self, game_object_id, game)
	self._game_object_id = game_object_id
	self._game = game
	self._is_client = true
end

PlayerUnitDamage.remove_game_object_id = function (self)
	self._game_object_id = nil
	self._game = nil
	self._is_client = nil
end

PlayerUnitDamage.network_recieve_add_damage = function (self, attacker_player, attacker_unit, damage_type, damage, position, normal, damage_range_type, gear_name, hit_zone, impact_direction, real_damage, range, mirrored)
...
PlayerUnitDamage.add_damage = function (self, attacking_player, attacking_unit, damage_type, damage, position, normal, actor, damage_range_type, gear_name, hit_zone, impact_direction, real_damage, range, mirrored)
...
PlayerUnitDamage.network_recieve_add_damage_over_time = function (self, attacking_player, damage_type)
	self:add_damage_over_time(attacking_player, damage_type)
end

PlayerUnitDamage.add_damage_over_time = function (self, attacking_player, damage_type)
...
PlayerUnitDamage._update_dot_effects = function (self)
	if self._game then
...
PlayerUnitDamage.play_dot_effect = function (self, damage_over_time_type, effect_name, node_name)
...
PlayerUnitDamage.stop_dot_effect = function (self, damage_over_time_type)
...
PlayerUnitDamage.update = function (self, unit, input, dt, context, t)
...
PlayerUnitDamage._client_update = function (self, t, dt)
...
PlayerUnitDamage._server_update = function (self, t, dt)
	local locomotion = ScriptUnit.extension(self._unit, "locomotion_system")
...
PlayerUnitDamage._update_health = function (self, t, dt)
	if self._game and self._game_object_id then
...
PlayerUnitDamage._has_damage_over_time_applied = function (self)
...
PlayerUnitDamage._calculate_current_dot_damage = function (self, t, dt)
...
PlayerUnitDamage.self_knock_down = function (self)
	if not self:is_dead() and not self:is_knocked_down() then
		self:knocked_down(self._player, nil, nil, Vector3(0, 0, 0), "kinetic")
	end
end

PlayerUnitDamage.knocked_down = function (self, attacker, gear_name, hit_zone, impact_direction, damage_type)
...
PlayerUnitDamage.can_yield = function (self)
	return self._knocked_down and not self._dead and not self._executed_by and not self._revived_by
end

PlayerUnitDamage.yield = function (self)
	self:die(self._player, nil, false, "yield")
end

PlayerUnitDamage.die = function (self, attacker, gear_name, is_instakill, damage_type, hit_zone, impact_direction)
	if not attacker or not attacker:alive() then
		attacker = nil
		gear_name = nil
	end
...
PlayerUnitDamage.is_wounded = function (self)
	return self._wounded
end

PlayerUnitDamage.is_dead = function (self)
	return self._dead
end

PlayerUnitDamage.is_knocked_down = function (self)
	return self._knocked_down
end

PlayerUnitDamage.is_alive = function (self)
	return not self._knocked_down and not self._dead
end

PlayerUnitDamage.destroy = function (self)
...
PlayerUnitDamage.player_dead = function (self)
...
PlayerUnitDamage.abort_execution = function (self, executor_id)
...
PlayerUnitDamage.can_be_bandaged = function (self)
	return not self._dead and not self._knocked_down and not self._bandaged_by
end

PlayerUnitDamage.can_be_revived = function (self)
	return not self._dead and self._knocked_down and not self._revived_by and not self._executed_by
end

PlayerUnitDamage.start_revive = function (self, reviver_id, revivee_id)
...
PlayerUnitDamage.start_bandage = function (self, bandager_id, bandagee_id)
...
PlayerUnitDamage.completed_revive = function (self)
	local locomotion_ext = ScriptUnit.extension(self._unit, "locomotion_system")
...
PlayerUnitDamage._clear_damagers = function (self)
	table.clear(self._damagers)
end

PlayerUnitDamage._clear_damage_over_time = function (self)
...
PlayerUnitDamage.rpc_bandage_completed_client = function (self)
	WeaponHelper:remove_projectiles(self._unit)
...
PlayerUnitDamage.completed_bandage = function (self)
...
PlayerUnitDamage.abort_bandage = function (self, bandager_id, bandagee_id)
...
PlayerUnitDamage.abort_revive = function (self, reviver_id, revivee_id)
...
PlayerUnitDamage._set_hud_blackboard_end_time = function (self, debuff_type, value)
	if not self._is_husk then
...
PlayerUnitDamage._increment_hud_blackboard_level = function (self, debuff_type, modifier)
	if not self._is_husk then
...
PlayerUnitDamage._update_buff_regeneration = function (self, t, dt)
	local area_buff_ext = ScriptUnit.extension(self._unit, "area_buff_system")
...
PlayerUnitDamage._set_debuff_blackboard_fields = function (self)
...
PlayerUnitDamage.debuff_blackboard = function (self)
