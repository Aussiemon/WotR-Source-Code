...
PlayerMovementStateBase.init = function (self, unit, internal)
	self._unit = unit
...
PlayerMovementStateBase._max_stamina = function (self, internal, encumbrance)
...
PlayerMovementStateBase._activate_parry_perks = function (self, hit_gear_unit, hitting_gear_unit, fully_charged_attack)
...
PlayerMovementStateBase.update = function (self, dt, t)
...
PlayerMovementStateBase._update_stamina = function (self, dt, t)
	return
end

PlayerMovementStateBase.post_update = function (self, dt)
...
PlayerMovementStateBase.enter = function (self, old_state)
	return
end

PlayerMovementStateBase.exit = function (self, new_state)
...
PlayerMovementStateBase.destroy = function (self)
	return
end

PlayerMovementStateBase.change_state = function (self, new_state, ...)
	local internal = self._internal
...
PlayerMovementStateBase.set_target_rotation = function (self, target_rot)
	local internal = self._internal
...
PlayerMovementStateBase.set_local_position = function (self, new_pos)
	local internal = self._internal
...
PlayerMovementStateBase.set_local_rotation = function (self, new_rot)
	local internal = self._internal
...
PlayerMovementStateBase.aim_direction = function (self)
	local unit = self._unit
...
PlayerMovementStateBase.aim_rotation = function (self)
...
PlayerMovementStateBase.update_camera_target = function (self, dt)
	local internal = self._internal
...
PlayerMovementStateBase.update_camera = function (self, dt)
...
PlayerMovementStateBase.update_lerped_rotation = function (self, dt)
	local unit = self._unit
	local internal = self._internal
...
PlayerMovementStateBase.set_target_world_rotation = function (self, target_rot)
	local internal = self._internal
...
PlayerMovementStateBase._update_current_rotation = function (self, dt)
	local internal = self._internal
...
PlayerMovementStateBase.set_world_rotation = function (self, new_rot)
	self:set_target_world_rotation(new_rot)
	self:_set_rotation(new_rot)
end

PlayerMovementStateBase._set_rotation = function (self, new_rot)
	local unit = self._unit
	local internal = self._internal
...
PlayerMovementStateBase.anim_event = function (self, event, force_local)
	local unit = self._unit
...
PlayerMovementStateBase.anim_event_with_variable_float = function (self, event, variable_name, variable_value, force_local)
	local unit = self._unit
...
PlayerMovementStateBase.bow_draw_animation_event = function (self, event, draw_time, tense_time, hold_time)
	local unit = self._unit
...
PlayerMovementStateBase.update_aim_rotation = function (self, dt, t)
...
PlayerMovementStateBase._move_speed = function (self)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase.update_look_angle = function (self, dt)
	local unit = self._unit
	local camera_manager = Managers.state.camera
...
PlayerMovementStateBase.update_look_anim_var = function (self, dt)
	local internal = self._internal
	local unit = self._unit
...
PlayerMovementStateBase.update_aim_target = function (self, dt)
	local unit = self._unit
	local camera_manager = Managers.state.camera
	local viewport_name = Unit.get_data(unit, "viewport_name")
...
PlayerMovementStateBase._update_officer_buff_activation = function (self, dt, t)
	local internal = self._internal
...
PlayerMovementStateBase._update_weapons = function (self, dt, t)
...
PlayerMovementStateBase._update_block = function (self, dt, t)
	local internal = self._internal
	local controller = self._controller
...
PlayerMovementStateBase._update_parry_attempt = function (self, dt, slot_name, block_type)
...
PlayerMovementStateBase._update_parry_attempt_keyboard = function (self, dt, slot_name, block_type)
...
PlayerMovementStateBase._update_parry_attempt_mouse = function (self, dt, slot_name, block_type)
	local internal = self._internal
...
PlayerMovementStateBase._update_swing = function (self, dt, t)
	local internal = self._internal
	local controller = self._controller
...
PlayerMovementStateBase._swing_ready = function (self, t)
	local internal = self._internal
...
PlayerMovementStateBase._update_pose_attempt = function (self, dt)
...
PlayerMovementStateBase._update_pose_attempt_keyboard = function (self, dt)
	local internal = self._internal
	local move = self._controller and self._controller:get("move")
...
PlayerMovementStateBase._update_pose_attempt_mouse = function (self, dt)
	local internal = self._internal
...
PlayerMovementStateBase._update_pose = function (self, dt, t)
	local internal = self._internal
...
PlayerMovementStateBase._update_ranged_weapons = function (self, dt, t)
	local controller = self._controller
...
PlayerMovementStateBase._update_weapon_wield = function (self, dt, t)
...
PlayerMovementStateBase._update_tagging = function (self, dt, t)
	local internal = self._internal
	local controller = self._controller
...
PlayerMovementStateBase._process_tag = function (self, player, unit_being_tagged)
	local internal = self._internal
	local network_manager = Managers.state.network
...
PlayerMovementStateBase.tagging_raycast_cb = function (self, hits)
	local internal = self._internal
	local tagged_unit
...
PlayerMovementStateBase._tag_valid = function (self, tagged_unit)
	local player = self._internal.player
...
PlayerMovementStateBase._update_tagging_nothing = function (self)
	return
end

PlayerMovementStateBase._can_switch_weapon_grip = function (self, t)
	local internal = self._internal
...
PlayerMovementStateBase._can_tag = function (self, t, tagging_player)
	local internal = self._internal

	return t >= internal.tagging_cooldown and tagging_player.squad_index and not internal.tagging and not internal.ghost_mode
end

PlayerMovementStateBase._can_activate_officer_buff = function (self, buff_index, t)
	return false
end

PlayerMovementStateBase.can_wield_weapon = function (self)
	return false
end

PlayerMovementStateBase.can_toggle_weapon = function (self)
	return false
end

PlayerMovementStateBase.can_attempt_pose_melee_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_pose_melee_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_swing_melee_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_abort_melee_swing = function (self)
	return false, nil
end

PlayerMovementStateBase.can_aim_ranged_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_unaim_ranged_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_fire_ranged_weapon = function (self)
	return false, nil
end

PlayerMovementStateBase.can_raise_block = function (self)
	return false, nil
end

PlayerMovementStateBase.can_lower_block = function (self)
	return false, nil
end

PlayerMovementStateBase.can_crouch = function (self)
	return false
end

PlayerMovementStateBase.can_rush = function (self)
	return false
end

PlayerMovementStateBase.can_mount = function (self)
	return false
end

PlayerMovementStateBase.can_unmount = function (self)
	return false
end

PlayerMovementStateBase.can_jump = function (self)
	return false
end

PlayerMovementStateBase.can_pickup_flag = function (self)
	return false
end

PlayerMovementStateBase.can_drop_flag = function (self)
	return false
end

PlayerMovementStateBase.can_plant_flag = function (self)
	return false
end

PlayerMovementStateBase.can_shield_bash = function (self)
	return false
end

PlayerMovementStateBase.can_push = function (self)
	return false
end

PlayerMovementStateBase.can_call_horse = function (self)
	return false
end

PlayerMovementStateBase._dual_wield_right_hand_fallback = function (self)
	local fallback_slot
	local inventory = self._internal:inventory()
...
PlayerMovementStateBase._wield_weapon = function (self, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._unwield_slots_on_wield = function (self, wield_slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._set_slot_wielded_instant = function (self, slot_name, wielded)
	local inventory = self._internal:inventory()
...
PlayerMovementStateBase.anim_cb_wield = function (self)
	local internal = self._internal
...
PlayerMovementStateBase.anim_cb_wield_finished = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._pose_melee_weapon = function (self, slot_name, t)
	local internal = self._internal
	local inventory = internal:inventory()
	local unit = self._unit
	local gear = inventory:_gear(slot_name)
...
PlayerMovementStateBase._play_voice = function (self, event)
	local internal = self._internal
...
PlayerMovementStateBase._swing_melee_weapon = function (self, t)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase.anim_cb_swing_finished = function (self)
...
PlayerMovementStateBase.gear_cb_abort_swing = function (self, reason)
	self:_abort_swing(reason)
end

PlayerMovementStateBase.gear_cb_abort_shield_bash_swing = function (self, reason)
	self:_abort_shield_bash_swing(reason)
end

PlayerMovementStateBase.gear_cb_abort_push_swing = function (self, reason)
	self:_abort_push_swing(reason)
end

PlayerMovementStateBase.swing_finished = function (self, reason)
	local internal = self._internal
...
PlayerMovementStateBase.shield_bash_swing_finished = function (self, reason)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase.push_swing_finished = function (self, reason)
	local internal = self._internal
	local inventory = internal:inventory()
	local swing_recovery = inventory:end_melee_attack(inventory:wielded_block_slot(), reason)
...
PlayerMovementStateBase._aim_ranged_weapon = function (self, slot_name, t)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._unaim_ranged_weapon = function (self)
	local internal = self._internal
	local inventory = internal:inventory()
	local slot_name = inventory:wielded_ranged_weapon_slot()
	local gear = inventory:_gear(slot_name)
...
PlayerMovementStateBase._fire_ranged_weapon = function (self, t)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._firing_event_callback = function (self, gear_category)
...
PlayerMovementStateBase._start_reloading_bow_on_wield_or_new_ammo = function (self, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._start_reloading_bow_on_release = function (self, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
	local reload_time = inventory:ranged_weapon_reload_time(slot_name)
...
PlayerMovementStateBase._start_reloading_bow = function (self, dt, t, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
	local reload_time = inventory:ranged_weapon_reload_time(slot_name)
	local gear = inventory:_gear(slot_name)
...
PlayerMovementStateBase._start_reloading_crossbow = function (self, dt, t, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._start_reloading_handgonne = function (self, dt, t, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMovementStateBase._start_reloading_weapon = function (self, dt, t, slot_name)
	local inventory = self._internal:inventory()
	local gear = inventory:_gear(slot_name)
	local extensions = gear:extensions()
	local weapon_ext = extensions.base
...
PlayerMovementStateBase._finish_reloading_weapon = function (self, reloading_successful)
	local internal = self._internal
	local inventory = internal:inventory()
	local slot_name = inventory:wielded_ranged_weapon_slot()
	local gear = inventory:_gear(slot_name)
...
PlayerMovementStateBase.anim_cb_ready_arrow = function (self)
	local internal = self._internal
...
PlayerMovementStateBase.anim_cb_bow_ready = function (self)
	local internal = self._internal
	local inventory = internal:inventory()
	local gear = inventory:_gear(internal.reload_slot_name)
	local extensions = gear:extensions()
...
PlayerMovementStateBase.anim_cb_bow_action_finished = function (self)
	self:anim_event("bow_action_finished")
end

PlayerMovementStateBase._raise_shield_block = function (self, slot_name)
	local internal = self._internal
	local network_manager = Managers.state.network
	local unit = self._unit
...
PlayerMovementStateBase._raise_parry_block = function (self, slot_name, block_type, direction)
...
PlayerMovementStateBase._lower_block = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._lower_shield_block = function (self)
	local internal = self._internal
	local network_manager = Managers.state.network
	local unit = self._unit
...
PlayerMovementStateBase._lower_parry_block = function (self, slot_name)
	local internal = self._internal
	local inventory = internal:inventory()
	local network_manager = Managers.state.network
	local unit = self._unit
...
PlayerMovementStateBase._convert_swing_to_parry_direction = function (self)
...
PlayerMovementStateBase.safe_action_interrupt = function (self, interrupting_action)
	local internal = self._internal
	local t = Managers.time:time("game")
...
PlayerMovementStateBase._end_swing_recovery = function (self)
	self._internal.swing_recovery_time = nil

	self:anim_event("swing_attack_penalty_finished")
end

PlayerMovementStateBase._abort_pose = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._end_pose = function (self)
	local internal = self._internal
	local network_manager = Managers.state.network
...
PlayerMovementStateBase._abort_parry_attempt = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._abort_swing = function (self, reason)
...
PlayerMovementStateBase._abort_shield_bash_swing = function (self, reason)
	self:shield_bash_swing_finished(reason)
end

PlayerMovementStateBase._abort_shield_bash_pose = function (self)
	return
end

PlayerMovementStateBase._abort_push_swing = function (self, reason)
	self:push_swing_finished(reason)
end

PlayerMovementStateBase._abort_calling_horse = function (self)
	self:finish_calling_horse("interupted")
end

PlayerMovementStateBase._abort_reload = function (self)
	self:_finish_reloading_weapon(false)
end

PlayerMovementStateBase._abort_crouch = function (self)
	self:anim_event("to_uncrouch")

	self._internal.crouching = false
end

PlayerMovementStateBase._abort_officer_buff_activation = function (self, progress)
	local internal = self._internal
...
PlayerMovementStateBase.wielded_weapon_destroyed = function (self, slot_name)
...
PlayerMovementStateBase._abort_wield = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._abort_tagging = function (self)
	local internal = self._internal
...
PlayerMovementStateBase._drop_flag = function (self)
	local internal = self._internal
...
PlayerMovementStateBase.update_wounded_camera_effect = function (self, dt, t)
	local unit = self._unit
	local internal = self._internal
...
PlayerMovementStateBase.update_switch_weapon_grip = function (self, dt, t)
	local controller = self._controller
	local inventory = self._internal:inventory()
...
PlayerMovementStateBase.update_call_horse_decals = function (self, dt, t)
	local internal = self._internal
...
PlayerMovementStateBase.update_parry_helper = function (self, dt, t)
...
PlayerMovementStateBase.cb_near_human = function (self, actors)
...
PlayerMovementStateBase._calculate_closest_enemy = function (self, actors)
	local units = {}
...
PlayerMovementStateBase._looking_at_eachother = function (self, target_unit, target_locomotion)
...
PlayerMovementStateBase._update_ranged_weapon_zoom = function (self, dt, t)
	local internal = self._internal
...
PlayerMovementStateBase._update_breathing_state = function (self, dt, t, slot_name)
	local controller = self._controller
...
PlayerMovementStateBase._update_sway_camera = function (self, dt, t, settings, current_breathing_state, next_breathing_state)
	local internal = self._internal
...
PlayerMovementStateBase._calculate_transition_increments = function (self, t, settings, current_breathing_state, next_breathing_state, transition_time)
...
PlayerMovementStateBase._update_breathing_transition = function (self, dt)
	local internal = self._internal
...
PlayerMovementStateBase._increment_angle = function (self, dt, angle, increment_value, sway_time)
...
PlayerMovementStateBase._calculate_pitch_speed = function (self)
	local internal = self._internal
...
PlayerMovementStateBase.anim_cb_knockdown_finished = function (self)
	return
end

PlayerMovementStateBase._update_breathing_sounds = function (self, angle, state)
	local internal = self._internal
