...
PlayerKnockedDown.init = function (self, unit, internal, world)
...
PlayerKnockedDown.update = function (self, dt, t)
...
PlayerKnockedDown.anim_cb_knockdown_finished = function (self)
	self._falling = false
end

PlayerKnockedDown.update_execution = function (self, execution_t, dt)
...
PlayerKnockedDown.begin_execution = function (self, execution_definition, attacker_unit)
...
PlayerKnockedDown.abort_execution = function (self)
	local internal = self._internal
...
PlayerKnockedDown.yield_confirmed = function (self)
	self:anim_event("yield")

	self._internal.yielded = true
end

PlayerKnockedDown.yield_denied = function (self)
	self._yielded = false
end

PlayerKnockedDown.can_yield = function (self, t)
	local internal = self._internal

	return InteractionHelper:can_request("yield", internal) and not self._yielded
end

PlayerKnockedDown.update_yield = function (self, dt, t)
...
PlayerKnockedDown.update_movement = function (self, dt)
...
PlayerKnockedDown.start_revive = function (self)
	self:anim_event_with_variable_float("revive_start", "revive_time", PlayerUnitDamageSettings.REVIVE_TIME)
end

PlayerKnockedDown.abort_revive = function (self)
	self:anim_event("revive_abort")
end

PlayerKnockedDown.revived = function (self)
	self:anim_event("revive_complete")
	self:recover_wielded_gear()
	self:change_state("onground")
end

PlayerKnockedDown.enter = function (self, old_state, hit_zone, impact_direction, damage_type)
...
PlayerKnockedDown.drop_wielded_gear = function (self)
	self._internal:inventory():set_kinematic_wielded_gear(false)
end

PlayerKnockedDown.recover_wielded_gear = function (self)
	self._internal:inventory():set_kinematic_wielded_gear(true)
end

PlayerKnockedDown._pick_knock_down_anim_event = function (self, hit_zone, impact_direction, damage_type)
...
PlayerKnockedDown._calculate_knock_down_direction = function (self, impact_direction)
	local internal = self._internal
...
PlayerKnockedDown._can_tag = function (self, t, tagging_player)
	local internal = self._internal

	return t >= internal.kd_tagging_cooldown and not internal.tagging
end

PlayerKnockedDown._tag_valid = function (self, tagged_unit)
	local player = self._internal.player
...
PlayerKnockedDown._process_tag = function (self, player, tagged_unit)
	local internal = self._internal
	local network_manager = Managers.state.network
...
PlayerKnockedDown._update_tagging_nothing = function (self, dt, t)
	local internal = self._internal
...
PlayerKnockedDown.exit = function (self, new_state)
...
PlayerKnockedDown.destroy = function (self)
