...
PlayerMounted.init = function (self, ...)
...
PlayerMounted.update = function (self, dt, t)
...
PlayerMounted._update_fall_height = function (self, dt, t)
...
PlayerMounted.stun = function (self, hit_zone, impact_direction, impact_type, speed)
...
PlayerMounted._update_stamina = function (self, dt, t)
	local internal = self._internal
...
PlayerMounted._aim_ranged_weapon = function (self, ...)
...
PlayerMounted._unaim_ranged_weapon = function (self, ...)
...
PlayerMounted._calculate_new_lean_state = function (self, dt, t, look_angle, direction)
...
PlayerMounted._set_game_object_lean_values = function (self, lean_value, swing_height)
	local internal = self._internal

	GameSession.set_game_object_field(internal.game, internal.id, "pose_lean_anim_blend_value", Vector3(lean_value, swing_height, 0))
end

PlayerMounted.update_left = function (self, dt, t, unit, look_angle)
...
PlayerMounted.update_left_border = function (self, dt, t, unit, look_angle)
...
PlayerMounted.update_right = function (self, dt, t, unit)
	local swing_height = self:_swing_height()
...
PlayerMounted.update_right_border = function (self, dt, t, unit, look_angle)
	local look_t = math.smoothstep(look_angle, -LOOK_ANGLE_DEAD_ZONE_START, -LOOK_ANGLE_DEAD_ZONE)
	local swing_height = math.lerp(self:_swing_height(), 1, look_t)
...
PlayerMounted.update_middle = function (self, dt, t, unit, look_angle)
...
PlayerMounted.enter_left = function (self, old_state)
	self:anim_event("lean")
end

PlayerMounted.enter_left_border = function (self, old_state)
	self:anim_event("lean")
end

PlayerMounted.enter_right = function (self, old_state)
	self:anim_event("lean")
end

PlayerMounted.enter_right_border = function (self, old_state)
	self:anim_event("lean")
end

PlayerMounted.enter_middle = function (self, old_state)
	self:anim_event("unlean")
end

PlayerMounted._swing_height = function (self)
	local internal = self._internal
...
PlayerMounted._update_lean_swing_variable = function (self, dt, t)
	local internal = self._internal
	local unit = self._unit
	local internal = self._internal
...
PlayerMounted.enter = function (self, old_state)
...
PlayerMounted.can_crouch = function (self, t)
	local internal = self._internal
...
PlayerMounted._update_crouch = function (self, dt, t)
	local internal = self._internal
...
PlayerMounted._crouch = function (self)
	self:anim_event("to_crouch")

	self._internal.crouching = true
end

PlayerMounted.force_unmount = function (self, reason)
	if self._fall_height then
		self:change_state("inair", self._fall_height)
	else
...
PlayerMounted.anim_cb_forceful_dismount_anim_finished = function (self)
	self._forceful_dismount_anim_finished = true
end

PlayerMounted.anim_cb_mounting_complete = function (self)
	local internal = self._internal
...
PlayerMounted.exit = function (self, new_state)
...
PlayerMounted._unmount_husk = function (self, mount, internal)
	local player_manager = Managers.player
...
PlayerMounted._update_velocity = function (self, dt, t)
	local internal = self._internal

	internal.velocity:store(ScriptUnit.extension(internal.mounted_unit, "locomotion_system"):get_velocity())
end

PlayerMounted._update_movement_mount_dead = function (self, dt, t)
	local unit = self._unit
	local mover = Unit.mover(unit)
...
PlayerMounted._update_movement = function (self, dt, t)
	local unit = self._unit
	local mover = Unit.mover(unit)
	local wanted_pose = Unit.animation_wanted_root_pose(unit)
	local wanted_position = Matrix4x4.translation(wanted_pose)
...
PlayerMounted._update_mover_position = function (self, dt, t)
	local internal = self._internal
	local unit = self._unit
	local mover = Unit.mover(unit)
...
PlayerMounted.update_aim_target = function (self, dt)
	local internal = self._internal
	local unit = self._unit
...
PlayerMounted.aim_direction = function (self)
	local internal = self._internal
	local aim_target = internal.aim_target:unbox()
	local mounted_unit = internal.mounted_unit
	local unit = self._unit
...
PlayerMounted._update_swing = function (self, dt, t)
	local internal = self._internal
...
PlayerMounted._check_interact = function (self, dt, t)
...
PlayerMounted._enter_dismount = function (self, t)
	local internal = self._internal
...
PlayerMounted._play_dismount_animation = function (self, mount)
...
PlayerMounted.anim_cb_dismounting_complete = function (self)
	self:_set_transition()
end

PlayerMounted._set_transition = function (self)
	if self._stunned then
...
PlayerMounted._update_transition = function (self, dt, t)
...
PlayerMounted._begin_couch = function (self, t, couch_slot)
...
PlayerMounted._update_couch = function (self, dt, t)
	local internal = self._internal
	local unit = self._unit
...
PlayerMounted.post_world_update = function (self, dt, t)
	local internal = self._internal
...
PlayerMounted.anim_cb_enter_couch = function (self)
...
PlayerMounted.gear_cb_abort_couch = function (self, reason)
	assert(self._internal.couching)
	self:anim_event("hit_reaction_lance_impact")
	self:_end_couch(Managers.time:time("game"))
end

PlayerMounted._end_couch = function (self, t)
	local internal = self._internal
	local inventory = internal:inventory()
...
PlayerMounted.begin_charge = function (self, t)
...
PlayerMounted.end_charge = function (self)
