...
PlayerUnitInventory.init = function (self, world, user_unit, player, player_unit_game_object_id, tint_color, secondary_tint_color)
...
PlayerUnitInventory.fallback_slot = function (self)
...
PlayerUnitInventory.add_armour = function (self, armour_name, pattern_index, encumbrance_multiplier)
...
PlayerUnitInventory.gear_dead = function (self, slot_name, attachments)
...
PlayerUnitInventory._add_rejuvenating_gear = function (self, slot_name, gear_name, attachments)
...
PlayerUnitInventory._rejuvenate_gear = function (self, slot_name, rejuvenation_data)
	local locomotion = ScriptUnit.extension(self._user_unit, "locomotion_system")
...
PlayerUnitInventory.update = function (self, dt, t)
...
PlayerUnitInventory.bandage_rejuvenate_gear = function (self)
	for name, slot in pairs(self._slots) do
		local rejuvenation_data = self._rejuvenating_gear[name]
		local is_lance = rejuvenation_data and (rejuvenation_data.gear_name == "tournament_lance" or rejuvenation_data.gear_name == "war_lance")
...
PlayerUnitInventory.add_gear = function (self, gear_name, obj_id, ai_gear, attachments, locomotion)
...
PlayerUnitInventory.remove_gear = function (self, slot_name)
...
PlayerUnitInventory.add_helmet = function (self, helmet_name, team_name)
...
PlayerUnitInventory.add_helmet_attachment = function (self, helmet_name, attachment_type, attachment_name, team_name)
...
PlayerUnitInventory.add_helmet_crest = function (self, crest_name, team_name)
...
PlayerUnitInventory.encumbrance = function (self)
	local locomotion = ScriptUnit.extension(self._user_unit, "locomotion_system")
...
PlayerUnitInventory.can_wield = function (self, slot_name, player_state_name)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.can_unwield = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:wielded()
end

PlayerUnitInventory.can_toggle = function (self, slot_name)
	return InventorySlots[slot_name].wield_toggle
end

PlayerUnitInventory.is_wielded = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:wielded()
end

PlayerUnitInventory.is_equipped = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and true
end

PlayerUnitInventory.is_two_handed = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:is_two_handed()
end

PlayerUnitInventory.is_one_handed = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:is_one_handed()
end

PlayerUnitInventory.is_left_handed = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:is_left_handed()
end

PlayerUnitInventory.is_right_handed = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:is_right_handed()
end

PlayerUnitInventory.wielded_weapon_slot = function (self)
...
PlayerUnitInventory.wielded_melee_weapon_slot = function (self)
	for slot_name, slot in pairs(self._slots) do
		local gear = slot.gear
...
PlayerUnitInventory.wielded_block_slot = function (self)
...
PlayerUnitInventory.allows_couch = function (self)
	for slot_name, slot in pairs(self._slots) do
		local gear = slot.gear
...
PlayerUnitInventory.gear_couch_settings = function (self, slot_name)
	return self:_gear(slot_name):settings().attacks.couch
end

PlayerUnitInventory.gear_unit = function (self, slot_name)
	return self:_gear(slot_name):unit()
end

PlayerUnitInventory.start_melee_attack = function (self, slot_name, charge_time, swing_direction, gear_cb_abort_swing, swing_time_multiplier)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.start_couch = function (self, slot_name, abort_func)
	local gear = self:_gear(slot_name)

	gear:start_couch(abort_func)
end

PlayerUnitInventory.end_couch = function (self, slot_name)
	local gear = self:_gear(slot_name)

	gear:end_couch()
end

PlayerUnitInventory.end_melee_attack = function (self, slot_name, reason)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.can_block = function (self, slot_name)
	local gear = self:_gear(slot_name)

	return gear and gear:wielded() and gear:can_block()
end

PlayerUnitInventory.can_reload = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local extensions = gear:extensions()
	local weapon_ext = extensions and extensions.base
...
PlayerUnitInventory.block_priority = function (self, slot_name)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.block_type = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
...
PlayerUnitInventory.weapon_wield_time = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
	local locomotion = ScriptUnit.extension(self._user_unit, "locomotion_system")
...
PlayerUnitInventory.weapon_uncharged_damage_factor = function (self, slot_name, swing_direction)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
...
PlayerUnitInventory.weapon_pose_movement_multiplier = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
	local locomotion = ScriptUnit.extension(self._user_unit, "locomotion_system")
...
PlayerUnitInventory.ranged_weapon_draw_time = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
...
PlayerUnitInventory.ranged_weapon_reload_time = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = gear:settings()
	local locomotion = ScriptUnit.extension(self._user_unit, "locomotion_system")
...
PlayerUnitInventory.unwield_slots_on_wield = function (self, wield_slot_name)
...
PlayerUnitInventory.weapon_wield_anim = function (self, slot_name)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.set_gear_wielded = function (self, slot_name, wielded, ignore_sound)
	local gear = self:_gear(slot_name)
...
PlayerUnitInventory.enter_ghost_mode = function (self)
...
PlayerUnitInventory.exit_ghost_mode = function (self)
	local slots = self._slots
...
PlayerUnitInventory.wield_reload_anim = function (self, slot_name)
	local gear = self:_gear(slot_name)
	local gear_settings = Gear[gear:name()]
...
PlayerUnitInventory.toggle_visor = function (self)
...
PlayerUnitInventory.weapon_grip_switched = function (self)
	return self._weapon_grip_switched
end

PlayerUnitInventory.set_grip_switched = function (self, switched)
	self._weapon_grip_switched = switched
end

PlayerUnitInventory.can_switch_weapon_grip = function (self)
