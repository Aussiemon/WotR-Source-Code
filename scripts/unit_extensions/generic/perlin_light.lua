...
PerlinLight.init = function (self, world, unit)
	self._world = world
	self._unit = unit
...
PerlinLight.update = function (self, unit, input, dt, context, t)
...
PerlinLight.destroy = function (self)
	return
end

PerlinLight._calculate_perlin_value = function (self, x, persistance, number_of_octaves, seed)
	local total = 0
	local max_value = 0
...
PerlinLight._interpolated_noise = function (self, x, seed)
...
PerlinLight._smoothed_noise = function (self, x, seed)
	return self:_noise(x, seed) / 2 + self:_noise(x - 1, seed) / 4 + self:_noise(x + 1, seed) / 4
end

PerlinLight._noise = function (self, x, seed)
