...
SpeedHackDetector.init = function (self, settings)
	if settings then
...
SpeedHackDetector.setup = function (self)
	if self._enabled then
		Managers.state.event:register(self, "check_speedhack", "event_check_speedhack")
	end
end

SpeedHackDetector.event_check_speedhack = function (self, player, round_time)
