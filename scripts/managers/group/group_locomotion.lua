...
GroupLocomotion.init = function (self, group, units, pos, rot)
	self._group = group
	self._units = units
	self._arrived = true
...
GroupLocomotion.position_rotation_from_offset = function (self, offset)
...
GroupLocomotion.unit_added = function (self, unit)
...
GroupLocomotion.unit_removed = function (self)
	return
end

GroupLocomotion.set_pose = function (self, pose)
	self._pose:store(pose)
end

GroupLocomotion.pose = function (self)
	return self._pose:unbox()
end

GroupLocomotion.position = function (self)
	local pose = self:pose()

	return Matrix4x4.translation(pose)
end

GroupLocomotion.set_move_target = function (self, wanted_pos, arrive_callback)
	self._arrived = false
...
GroupLocomotion.update = function (self, dt, t)
...
GroupLocomotion._debug_draw = function (self)
...
GroupLocomotion._update_group_rotation = function (self, dt, t)
...
GroupLocomotion._update_group_movement = function (self, dt, t)
	local move_target = self._move_target:unbox()
	local current_pose = self:pose()
	local current_pos = Matrix4x4.translation(current_pose)
...
GroupLocomotion.cb_unit_arrived = function (self, unit)
	local group_pose = self:pose()
...
GroupLocomotion._update_units_movement = function (self, dt, t)
