...
HUDCompassUnitIcon.init = function (self, config)
...
HUDCompassUnitIcon.width = function (self)
	return self._width
end

HUDCompassUnitIcon.height = function (self)
	return self._height
end

HUDCompassUnitIcon.update_size = function (self, dt, t, gui, layout_settings)
...
HUDCompassUnitIcon.update_position = function (self, dt, t, layout_settings, x, y, z)
	local config = self.config
...
HUDCompassUnitIcon.render = function (self, dt, t, gui, layout_settings)
	local config = self.config
	local icon_name = Unit.get_data(config.unit, "hud", "icon_name")
...
HUDCompassUnitIcon._render_icon = function (self, dt, t, gui, layout_settings, icon_name)
	local config = self.config
	local icon_settings = layout_settings.icons[icon_name]
	local texture = icon_settings.texture_func and icon_settings.texture_func(config.blackboard, config.player, config.unit, self._world)
...
HUDCompassUnitIcon._color = function (self, player, settings, blackboard)
	local color_table = {
		255,
		255,
		255,
...
HUDCompassUnitIcon.create_from_config = function (config)
