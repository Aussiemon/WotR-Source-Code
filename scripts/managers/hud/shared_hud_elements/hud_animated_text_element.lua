...
HUDAnimatedTextElement.init = function (self, config)
	self._width = nil
	self._height = nil
	self._t = 0
	self.config = config
...
HUDAnimatedTextElement.width = function (self)
	return self._width
end

HUDAnimatedTextElement.height = function (self)
	return self._height
end

HUDAnimatedTextElement.update_size = function (self, dt, t, gui, layout_settings)
	self._t = self._t + dt
...
HUDAnimatedTextElement.update_position = function (self, dt, t, layout_settings, x, y, z)
	self._x = x + (self.offset_x or 0)
	self._y = y + (self.offset_y or 0)
	self._z = z
end

HUDAnimatedTextElement.render = function (self, dt, t, gui, layout_settings)
	local config = self.config
	local font = layout_settings.font or MenuSettings.fonts.menu_font
	local font_size = layout_settings.font_size * (self.font_size_multiplier or 1)
	local line_height = layout_settings.line_height or self._text[1] and self._text[1].height or 0
...
HUDAnimatedTextElement.create_from_config = function (config)
