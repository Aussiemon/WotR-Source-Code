...
HUDParryHelperElement.init = function (self, config)
...
HUDParryHelperElement.update_position = function (self, dt, t, layout_settings, x, y, z)
...
HUDParryHelperElement.render = function (self, dt, t, gui, layout_settings)
	local config = self.config
	local blackboard = config.blackboard
	local rot_angle = blackboard and blackboard[config.name]
...
HUDParryHelperElement._color = function (self)
	local color_table = self._element_color

	return Color(color_table[1], color_table[2], color_table[3], color_table[4])
end

HUDParryHelperElement._texture = function (self)
	return self._element_texture
end

HUDParryHelperElement.create_from_config = function (config)
