...
HUDManager.init = function (self, world)
	self._world = world
	self._huds = {}
	self._active = true
end

HUDManager.add_player = function (self, player, menu_world)
	self._huds[player] = {
...
HUDManager.set_active = function (self, active)
	self._active = active
...
HUDManager.active = function (self)
	return self._active
end

HUDManager.set_hud_enabled = function (self, name, enabled)
	for player, huds in pairs(self._huds) do
		huds[name]:set_enabled(enabled)
	end
end

HUDManager.set_huds_enabled_except = function (self, enabled, except)
	for player, huds in pairs(self._huds) do
		for hud_name, hud in pairs(huds) do
...
HUDManager.post_update = function (self, dt, t, player)
...
HUDManager.destroy = function (self)
	for player, huds in pairs(self._huds) do
		for hud_name, hud in pairs(huds) do
			hud:destroy()
		end
...
HUDManager.post = function (self, channel_name, name, text, color, display_time)
	for player, huds in pairs(self._huds) do
		huds.chat_window:post(channel_name, name, text, color, display_time)
	end
end

HUDManager.output_console_text = function (self, text, color)
	for player, huds in pairs(self._huds) do
