...
HUDDamageNumber.init = function (self, config)
	self.config = config
	self.font_settings = MenuSettings.fonts.hell_shark_36
end

HUDDamageNumber.width = function (self)
	return 0
end

HUDDamageNumber.height = function (self)
	return 0
end

HUDDamageNumber.update_size = function (self, dt, t, gui, layout_settings)
...
HUDDamageNumber.update_position = function (self, dt, t, layout_settings, x, y, z)
	local config = self.config
	local progression = (t - config.start_time) / 2
end

HUDDamageNumber.render = function (self, dt, t, gui, layout_settings)
	local config = self.config
	local progression = (t - config.start_time) / 2
...
HUDDamageNumber.create_from_config = function (config)
