...
HUDDamageNumbers.init = function (self, world, player)
...
HUDDamageNumbers.event_show_damage_number = function (self, player, damage_type, damage, position, damage_range_type, impact_direction, armour_type)
...
HUDDamageNumbers.cb_number_time_out = function (self, id)
	self._damage_numbers_container:remove_element(id)

	self._available_ids[id] = true
end

HUDDamageNumbers.event_damage_numbers_activated = function (self, player)
	if player == self._player then
		self._active = true
	end
end

HUDDamageNumbers.event_damage_numbers_deactivated = function (self, player)
	if player == self._player then
		self._active = false
	end
end

HUDDamageNumbers.post_update = function (self, dt, t)
...
HUDDamageNumbers.destroy = function (self)
