...
CommandParserManager.init = function (self)
	return
end

CommandParserManager.execute = function (self, command_line, player)
...
CommandParserManager._is_valid_command_line = function (self, command_line)
	if string.sub(command_line, 1, string.len(COMMAND_PREFIX)) == COMMAND_PREFIX then
		return true
	end
end

CommandParserManager._parse_command_line = function (self, command_line)
...
CommandParserManager.build_command_line = function (self, command, argument)
...
CommandParserManager.destroy = function (self)
	return
end

Commands = {}

Commands.script = function (self, lua_code, player)
...
Commands.lag = function (self, text)
...
Commands.location = function (self, text)
	Managers.state.event:trigger("location_print_requested")
end

Commands.say = function (self, text, player)
...
Commands.say_team = function (self, text, player)
...
Commands.say_admin = function (self, text, player)
...
Commands.kill = function (self, text, player)
...
Commands.console = function (self, args, player)
...
Commands.game_speed = function (self, args, player)
	if Application.build() ~= "dev" then
		return
	end
...
Commands.fov = function (self, args, player)
	if Application.build() ~= "dev" then
		return
	end
...
Commands.free_flight_settings = function (self, args, player)
...
Commands.rcon = function (self, args, player)
...
Commands.next_level = function (self)
	Managers.state.event:trigger("next_level")
end

Commands.list_players = function (self)
...
Commands.kick_player = function (self, args)
...
Commands.ban_player = function (self, args)
	if Managers.lobby.server then
		args = string.split(args, " ")
...
Commands.temp_ban_player = function (self, args)
	if Managers.lobby.server then
		args = string.split(args, " ")
...
Commands.unban_player = function (self, id_or_name)
...
Commands.kill_player = function (self, peer_id)
	if Managers.lobby.server then
		if Managers.state.network:is_valid_peer(peer_id) then
...
Commands.set_friendly_fire = function (self, config)
	if Managers.lobby.server then
...
Commands.vote_kick = function (self, peer_id, player)
	local network_manager = Managers.state.network
...
Commands.y = function (self, _, player)
	Managers.state.voting:client_vote("yes", player)
end

Commands.n = function (self, _, player)
	Managers.state.voting:client_vote("no", player)
end

Commands.reload_level = function (self)
	if Managers.lobby.server then
		Managers.state.event:trigger("reload_level")
	end
end

Commands.change_level = function (self, map_pair)
	if Managers.lobby.server then
...
Commands.save_progress = function (self, flag)
	if Managers.lobby.server then
...
Commands.move_player = function (self, peer_team_pair)
	if Managers.lobby.server then
...
Commands.add_reserved_slot = function (self, peer_id)
	if Managers.lobby.server then
		if not peer_id then
			return true, "Player ID missing"
		end
...
Commands.remove_reserved_slot = function (self, peer_id)
	if Managers.lobby.server then
		if not peer_id then
			return true, "Player ID missing"
		end
...
Commands.login = function (self, _, player)
	if Managers.lobby.server then
...
Commands.logout = function (self, _, player)
	if Managers.lobby.server then
		local peer_id = player:network_id()
		local rcon_admin = Managers.admin:rcon_admin()
...
Commands.list_levels = function (self)
	local network_manager = Managers.state.network
...
Commands.vote_map = function (self, map_pair, player)
