...
GameModeBattle.init = function (self, settings, world, ...)
...
GameModeBattle._allowed_spawns_spawn_owner = function (self, team)
	return self._spawned and 0 or 1
end

GameModeBattle.spawned = function (self)
	self._spawned = true
end

GameModeBattle.custom_spawning = function (self, settings)
	return BattleSpawning:new(settings, self)
end

GameModeBattle.team_alive = function (self, members, kd_timer, round_timer)
	local alive = false
	local kd = false
...
GameModeBattle.evaluate_end_conditions = function (self)
...
GameModeBattle.gm_event_battle_tiebreak = function (self)
	self._tie_break_started = true
end

GameModeBattle.objective = function (self, local_player)
...
GameModeBattle.own_score_announcement = function (self, local_player)
	local announcement = ""
...
GameModeBattle.enemy_score_announcement = function (self, local_player)
	local announcement = ""
	local param1, param2 = GameModeHelper:announcement_parameters(announcement, local_player, self._world)
...
GameModeBattle.hud_score_text = function (self, team_name)
	return Managers.state.team:team_by_name(team_name).score
end

GameModeBattle.hud_progress = function (self, local_player)
