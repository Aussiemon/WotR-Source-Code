...
GameModeManager.init = function (self, world, game_mode_key, level_key, win_score, time_limit, server_game_mode_scale)
	self._world = world
...
GameModeManager.force_limited_observer_cam = function (self, team)
	local locked = self._game_mode:force_limited_observer_cam(team)

	return locked
end

GameModeManager.team_locked_observer_cam = function (self, team)
	local allowed = self._game_mode:team_locked_observer_cam(team)

	return allowed
end

GameModeManager.squad_spawn_mode = function (self, team)
...
GameModeManager.squad_spawn_stun = function (self, team)
	local spawning = self._spawning
	local game = Managers.state.network:game()
...
GameModeManager.allowed_spawns = function (self, team)
	local spawns = self._game_mode:allowed_spawns(team)

	return spawns
end

GameModeManager.allow_ghost_talking = function (self)
	return GameModeSettings[self._game_mode_key].allow_ghost_talking
end

GameModeMusic = class(GameModeMusic)

GameModeMusic.init = function (self, level_key, start_state, start_volume)
	self._level_key = level_key
...
GameModeMusic._get_win_scale_criterias = function (self, game_mode_key, level_key)
...
GameModeMusic.update = function (self, dt, t, win_scale, game_mode_key, level_key)
...
GameModeMusic._set_volume = function (self, volume)
	local id = self._id
...
GameModeMusic._start_intense = function (self, from, t)
	self._state = "intense"
	self._volume = 0
	self._cooldown = t + GameModeMusicSettings.intense.minimum_play_time
	self._transition_state = nil
...
GameModeMusic._start_critical = function (self, from, t)
...
GameModeMusic._start_normal = function (self, from, t)
...
GameModeMusic.destroy = function (self)
	self:stop()
end

GameModeMusic.stop = function (self)
...
GameModeMusicSP.init = function (self)
	return
end

GameModeMusicSP.update = function (self, t, dt)
	return
end

GameModeMusicSP.stop = function (self)
	return
end

GameModeMusicSP.destroy = function (self)
	return
end
...
GameModeManager.rpc_set_team_object_set_visible = function (self, key, visibility)
...
GameModeManager._hot_join_synch_object_sets = function (self, sender, player)
...
GameModeManager.flow_cb_set_team_set_variation = function (self, side, set_name)
	local team_name
...
GameModeManager._set_object_set_team_material_variation = function (self, set_name, team_name)
...
GameModeManager.rpc_set_object_set_variation = function (self, set_key, team_name)
	local set_name = self._object_set_names[set_key]

	self:_set_object_set_team_material_variation(set_name, team_name)
end

GameModeManager.flow_cb_set_team_set_visibility = function (self, side, set_alias, visibility)
	local team_name
...
GameModeManager._set_team_object_set_visibility = function (self, set, visibility)
...
GameModeManager._init_game_mode = function (self, game_mode_key, win_score, time_limit)
...
GameModeManager.objective_unit_damage_multiplier = function (self, objective_unit, objective_active, damage_range_type, friendly_fire)
...
GameModeManager.next_spawn_time = function (self, player)
	return self._spawning:next_spawn_time(player)
end

GameModeManager.squad_screen_spawning = function (self)
...
GameModeManager.client_update = function (self, dt, t)
...
GameModeManager.ready_to_transition = function (self)
	self._ready_to_transition = true
end

GameModeManager.stop_game_mode_music = function (self)
	self._game_mode_music:stop()
end

GameModeManager.trigger_end = function (self, round_end, t)
...
GameModeManager.objective = function (self, local_player)
	return self._game_mode:objective(local_player)
end

GameModeManager.time_announcement = function (self, local_player)
	return self._game_mode:time_announcement(local_player)
end

GameModeManager.own_score_announcement = function (self, local_player)
	return self._game_mode:own_score_announcement(local_player)
end

GameModeManager.enemy_score_announcement = function (self, local_player)
	return self._game_mode:enemy_score_announcement(local_player)
end

GameModeManager.own_capture_point_announcement = function (self, local_player)
	return self._game_mode:own_capture_point_announcement(local_player)
end

GameModeManager.enemy_capture_point_announcement = function (self, local_player)
	return self._game_mode:enemy_capture_point_announcement(local_player)
end

GameModeManager.hud_score_text = function (self, team_name)
	return self._game_mode:hud_score_text(team_name)
end

GameModeManager.hud_progress = function (self, local_player)
	return self._game_mode:hud_progress(local_player)
end

GameModeManager.hud_timer_text = function (self)
	return self._game_mode:hud_timer_text()
end

GameModeManager.objective_units = function (self)
...
GameModeManager.num_owned_objectives = function (self, team_name)
...
GameModeManager.owned_objectives = function (self, team_name)
	local objective_units = self:objective_units()
	local team_side = Managers.state.team:side(team_name)
...
GameModeManager.not_owned_objectives = function (self, team_name)
	local objective_units = self:objective_units()
	local team_side = Managers.state.team:side(team_name)
	local objectives_table = {}
...
GameModeManager.active_objectives_by_side = function (self, team_side)
	local objective_units = self:objective_units()
...
GameModeManager.give_score_for_kill = function (self, scoring_table)
	self._game_mode:give_score_for_kill(scoring_table)
end

GameModeManager.give_score_for_flag_planted = function (self, scoring_table)
	self._game_mode:give_score_for_flag_planted(scoring_table)
end

GameModeManager.event_player_unit_died = function (self, dead_player, attacking_player, gear_name, is_instakill, damagers, damage_type)
...
GameModeManager.event_player_knocked_down = function (self, knocked_down_player, attacking_player, gear_name, damagers, damage_type)
	local level_event
	local game_mode_key = self:game_mode_key()
...
GameModeManager.event_flag_planted = function (self, planter_player, interactable_unit)
	self:trigger_event("flag_planted", planter_player, interactable_unit)
end

GameModeManager.event_objective_captured = function (self, capturing_player, captured_unit)
...
GameModeManager.event_objective_captured_assist = function (self, assist_player, captured_unit)
...
GameModeManager.event_section_cleared_payload = function (self, assist_player, payload_unit)
	Managers.state.stats_collector:section_cleared_payload(assist_player, payload_unit)
end

GameModeManager.event_player_bandaged = function (self, bandagee, bandager)
...
GameModeManager.event_player_damaged = function (self, damagee, damager, damage, gear_name, hit_zone, damage_range_type, range, mirrored)
...
GameModeManager.event_mount_unit_dead = function (self, attacker, rider)
	if not rider then
...
GameModeManager.event_player_dotted = function (self, victim, attacker, damage_type)
...
GameModeManager.game_mode_modified_score = function (self, score)
	return self._game_mode:modified_score(score)
end

GameModeManager.set_time_limit = function (self, t)
	self._game_mode:set_time_limit(t)
end

GameModeManager.modify_time_limit = function (self, t)
	self._game_mode:modify_time_limit(t)
end

GameModeManager.time_limit = function (self)
	return self._game_mode:time_limit()
end

GameModeManager.hot_join_synch = function (self, sender, player)
