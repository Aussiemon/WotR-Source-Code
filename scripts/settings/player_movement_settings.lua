...
PlayerUnitMovementSettings.functions.SIN = function (t)
	return math.sin(t)
end

PlayerUnitMovementSettings.functions.LOG = function (t)
	return math.log(t)
end

PlayerUnitMovementSettings.functions.QUAD = function (t)
	return t * t
end
...
PlayerUnitMovementSettings.movement_acceleration = function (dt, current_speed, target_speed, encumbrance_factor, acceleration_multiplier)
...
PlayerUnitMovementSettings.encumbrance.functions.pose_time = function (enc)
...
PlayerUnitMovementSettings.encumbrance.functions.reload_time = function (enc)
	if enc < ENC_LIGHT then
		return 1
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.movement_acceleration = function (enc)
	if enc < ENC_LIGHT then
		return 1.4
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.movement_speed = function (enc)
	if enc < ENC_LIGHT then
		return 1.1
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.double_time_speed = function (enc)
	if enc < ENC_LIGHT then
		return 0.97
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.charge_duration = function (enc)
	if enc < ENC_LIGHT then
		return 1.3
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.charge_cooldown = function (enc)
	if enc < ENC_LIGHT then
		return 0.95
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.charge_speed = function (enc)
	if enc < ENC_LIGHT then
		return 1.05
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.climbing_ladders_speed = function (enc)
	if enc < ENC_LIGHT then
		return 1.3
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 1.3, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.miss_penalty = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.not_penetrated_penalty = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.9, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.parried_penalty = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.9, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.hard_penalty = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.9, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.blocked_penalty = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.9, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.double_time_acceleration = function (enc)
	if enc < ENC_LIGHT then
		return 0.9
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.9, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.ammo_regen_rate = function (enc)
	if enc < ENC_LIGHT then
		return 1.5
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.wield_time = function (enc)
	if enc < ENC_LIGHT then
		return 0.8
	elseif enc < ENC_METER then
...
PlayerUnitMovementSettings.encumbrance.functions.stun_time = function (enc)
	if enc < ENC_LIGHT then
		return 0.8
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.8, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.heavy_landing_height = function (enc)
	if enc < ENC_LIGHT then
		return 1.5
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 1.5, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.stamina_regen = function (enc)
	if enc < ENC_LIGHT then
		return 1.05
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 1.05, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.stamina_max = function (enc)
	if enc < ENC_LIGHT then
		return 0.8
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 0.8, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.movement_speed_backwards = function (enc)
	if enc < ENC_LIGHT then
		return 1.1
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 1.1, 1, enc)
...
PlayerUnitMovementSettings.encumbrance.functions.movement_speed_strafe = function (enc)
	if enc < ENC_LIGHT then
		return 1.1
	elseif enc < ENC_METER then
		return math.auto_lerp(ENC_LIGHT, ENC_METER, 1.1, 1, enc)
